...
Browser admin for the live Sepolia ZkcaResolver. Every action is signed by the connected wallet (MetaMask or any injected provider) and broadcast directly to Sepolia — no private keys are stored or used by the UI.
...
1. **Register `zkcontext-<orgname>.eth`** on the [Sepolia ENS app](https://sepolia.app.ens.domains) (~5 min, free with a Sepolia faucet). The app sometimes registers without auto-wrapping — that's fine, the UI handles wrapping.
2. Open `http://localhost:3030`, connect the wallet that owns the name.
3. Click **"+ Create your organization"**, type the org name (the form prepends `zkcontext-` and appends `.eth`).
...
6. Each user subname (e.g. `aysel.zkcontext-istanbulhospital.eth`) is now visible in the ENS app, OpenSea, and the user's wallet — with all `zkca:*` text records served by our resolver.

## What the UI does

| Action | Contract call | Authorized signer |
|---|---|---|
| Register org | `ZkcaResolver.registerOrg(label)` | the wrapped owner of `<label>.eth` |
| Register user | `ZkcaResolver.registerUser(orgNode, label, addr, role, namespaces, maxTag, expiry)` | org admin |
| Edit user | `ZkcaResolver.updateUser(orgNode, label, role, namespaces, maxTag, expiry)` | org admin |
| Revoke user | `ZkcaResolver.revokeUser(orgNode, label)` | org admin |
| Set partners | `ZkcaResolver.setPartners(orgNode, csv)` | org admin |
| Set proof commitment | `ZkcaResolver.setProofCommitment(orgNode, label, commitment32)` | the user wallet only |
...
└── next.config.ts          turbopack root + transpiles @zkca/contracts-types
```
...
The contract surface is stable — adding new text record keys requires both a contract change and a TS update in `@zkca/contracts-types`.
