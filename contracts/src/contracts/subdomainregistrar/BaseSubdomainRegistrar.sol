...
abstract contract BaseSubdomainRegistrar {
    ...
    function _setupDomain(bytes32 node, ISubdomainPricer pricer, address beneficiary, bool active)
        internal
        virtual
        authorised(node)
    {
    ...
    function _register(
        ...
        wrapper.setSubnodeRecord(parentNode, label, newOwner, resolver, 0, fuses | PARENT_CANNOT_CONTROL, expiry);
    ...
    function _setRecords(bytes32 node, address resolver, bytes[] calldata records) internal {
        for (uint256 i = 0; i < records.length; i++) {
            ...
            resolver.functionCall(records[i], "SubdomainRegistrar: Failed to set Record");
        }
    }
